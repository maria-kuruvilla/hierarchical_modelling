...
write_csv(model_results_new, here("forestry_simulation", "model_200_simulation_fitting_result.csv"))
...
# for when we used alpha_mean = 5.4
model_results_high_alpha <- model_results_df %>% 
  group_by(simulation, data_model, fitting_model) %>%
  mutate(alpha = true_value[parameter == "alpha"],
         sigma = true_value[parameter == "sigma"],
         b_for = true_value[parameter == "b_for"]) %>%
...
write_csv(model_results_high_alpha, here("forestry_simulation", "model_200_simulation_w_high_alpha_fitting_result.csv"))


ggplot(model_results_high_alpha %>% 
         filter(parameter == "b_for"), aes(x = b_for, y = estimate_mean)) +
  geom_point(aes(color = alpha),alpha = 0.5, size = 2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_wrap(~ paste("Data model: ",data_model) + paste("Fitting model: ",fitting_model)) +
  labs(x = "True covariate effect", y = "Estimated covariate effect") +
  scale_color_gradient2(name = 'alpha',
                        low = pal[1], mid = 'gray', high = pal[5], midpoint = 5) +
  theme_classic() 

#save figure

ggsave(here("forestry_simulation", "b_for_estimate_vs_true_alpha_col_high_alpha.png"), width = 6, height = 3)
...
ggsave(here("forestry_simulation", "b_for_estimate_vs_true_sigma_col_high_alpha.png"), width = 6, height = 3)
...
ggsave(here("forestry_simulation", "alpha_estimate_vs_true_sigma_col_high_alpha.png"), width = 6, height = 3)
